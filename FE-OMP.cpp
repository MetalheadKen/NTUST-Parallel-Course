...
void toEdge(const outputImage &input, outputImage &output) {
    ...
    #pragma omp parallel for default(none) firstprivate(input, GX, GY) shared(output) collapse(2)
...
uint32_t CHT(const outputImage &input, accumulator& votes) {
    ...
    #pragma omp parallel reduction(+:total_votes) default(none) firstprivate(input) shared(votes)
    {
